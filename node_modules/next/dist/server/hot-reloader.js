...
class HotReloader {
    ...
    async run(req, res, parsedUrl) {
        ...
        const handlePageBundleRequest = async (res, parsedUrl) => {
            ...
            if (constants_1.BLOCKED_PAGES.indexOf(page) === -1) {
                try {
    ...
    addWsPort(configs) {
        configs[0].plugins.push(new webpack_1.default.DefinePlugin({
            'process.env.NEXT_WS_PORT': this.wsPort
        }));
    }
    async getWebpackConfig() {
        const pagePaths = await glob(`+(_app|_document|_error).+(${this.config.pageExtensions.join('|')})`, { cwd: path_1.join(this.dir, 'pages') });
    ...
    async start() {
        ...
        this.wsPort = await new Promise((resolve, reject) => {
            ...
            this.wss = new ws_1.default.Server({ port: websocketPort }, function (err) {
                if (err) {
                    return reject(err);
                }
    ...
    assignBuildTools({ webpackDevMiddleware, webpackHotMiddleware, onDemandEntries }) {
        ...
        this.middlewares = [
            ...
            error_overlay_middleware_1.default,
    ...
    async prepareBuildTools(multiCompiler) {
        ...
        const onDemandEntries = on_demand_entry_handler_1.default(webpackDevMiddleware, multiCompiler, Object.assign({ dir: this.dir, buildId: this.buildId, reload: this.reload.bind(this), pageExtensions: this.config.pageExtensions, wsPort: this.wsPort }, this.config.onDemandEntries));
        return {
            webpackDevMiddleware,
            webpackHotMiddleware,
            onDemandEntries
