...
const assets_size_plugin_1 = __importDefault(require("./webpack/plugins/assets-size-plugin"));
...
function optimizationConfig({ dev, isServer, totalPages, target }) {
    ...
    if (isServer && target === 'serverless') {
        return {
            splitChunks: false,
            minimizer: [
                new terser_webpack_plugin_1.default(Object.assign({}, terserPluginConfig, { terserOptions: {
                        compress: false,
                        mangle: false,
                        module: false,
    ...
    config.minimizer = [
        new terser_webpack_plugin_1.default(terserPluginConfig)
    ];
...
async function getBaseWebpackConfig(dir, { dev = false, isServer = false, buildId, config, target = 'server', entrypoints }) {
    ...
    let webpackConfig = {
        ...
        entry: async () => {
            return Object.assign({}, clientEntries, entrypoints);
        },
        output: {
            path: outputPath,
            filename: ({ chunk }) => {
        ...
        plugins: [
            ...
            dev && !isServer && new friendly_errors_webpack_plugin_1.default(),
            ...
            target !== 'serverless' && !isServer && !dev && new assets_size_plugin_1.default(buildId, distDir)
        ].filter(Boolean)
    };
    ...
    webpackConfig.entry = async () => {
        const entry = Object.assign({}, await originalEntry());
