...
function renderDocument(Document, { props, docProps, pathname, query, buildId, assetPrefix, runtimeConfig, nextExport, dynamicImportsIds, err, dev, staticMarkup, devFiles, files, dynamicImports, }) {
    return '<!DOCTYPE html>' + server_1.renderToStaticMarkup(react_1.default.createElement(Document, Object.assign({ __NEXT_DATA__: {
            props,
            page: pathname,
            query,
            buildId,
        ...
        }, staticMarkup: staticMarkup, devFiles: devFiles, files: files, dynamicImports: dynamicImports, assetPrefix: assetPrefix }, docProps)));
}
async function renderToHTML(req, res, pathname, query, renderOpts) {
    const { err, dev = false, staticMarkup = false, App, Document, Component, buildManifest, reactLoadableManifest, ErrorDebug } = renderOpts;
    ...
    if (utils_1.isResSent(res))
        return null;
    const dynamicImports = [...get_dynamic_import_bundles_1.getDynamicImportBundles(reactLoadableManifest, reactLoadableModules)];
